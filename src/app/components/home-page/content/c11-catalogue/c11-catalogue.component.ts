import { Subject, Subscription } from 'rxjs';
import { Component, OnInit, ViewChild, ElementRef } from '@angular/core';
import { AbstractControl, FormBuilder, FormControl, FormGroup, Validators } from '@angular/forms';
import * as moment from 'moment';
import { NgxSpinnerService } from 'ngx-spinner';
import { ApiService } from 'src/app/services/api-service/api.service';
import { CommonService } from 'src/app/services/common-service/common.service';
import Swal from 'sweetalert2';
import { debounceTime, distinctUntilChanged } from 'rxjs/operators';

@Component({
  selector: 'app-c11-catalogue',
  templateUrl: './c11-catalogue.component.html',
  styleUrls: ['./c11-catalogue.component.css']
})
export class C11CatalogueComponent implements OnInit {

  // list data
  listCatalogues: any = [];
  listProducts: any = [];
  listCheckboxCategory: any[] = [];

  // update pagination
  page: any = 1;
  pageIndex: number = 1;
  pageLength = 0;
  pageSize = 5;
  pageSizeOptions: number[] = [5, 10, 20, 50, 100];
  totalPage: number = 0;

  // sort filter
  fieldToSort: string = "id";
  fieldDirToSort: string = "desc";

  // form data
  formData = this.formBuilder.group({
    name: ['', [Validators.required, this.categoryNameValidate]],
    code: [''],
  });

  formDetail = this.formBuilder.group({
    id: [''],
    name: [''],
    code: [''],
    createdDate: [''],
    createdBy: [''],
    modifiedDate: [''],
    modifiedBy: ['']
  });

  formUpdate = this.formBuilder.group({
    id: [''],
    name: ['', [Validators.required, this.categoryNameValidate]],
    code: ['', Validators.required]
  });

  regexPattern = /\-?\d*\.?\d{1,2}/;
  formGoToPage = this.formBuilder.group({
    numberPage: [1, [Validators.pattern(this.regexPattern)]]
  })

  // number what
  W100_SELECT_ALL_CATEGORY_PAGING: number = 100;
  W101_INSERT_DATA_CATEGORY: number = 101;
  W102_UPDATE_DATA_CATEGORY: number = 102;
  W103_DELETE_DATA_CATEGORY: number = 103;
  W104_DETAIL_DATA_CATEGORY: number = 104;
  W105_CHECK_EXISTS_CATEGORY: number = 105;
  W106_SELECT_ALL_CATEGORY: number = 106;
  W107_DELETE_DATA_BY_ID: number = 107;
  W108_GET_DATA_BY_ID: number = 108;

  W300_FIND_ALL_DATA_BY_CATEGORY_ID: number = 300;

  constructor(
    private apiService: ApiService,
    private spinner: NgxSpinnerService,
    private formBuilder: FormBuilder,
    private commonService: CommonService,
  ) { }

  ngOnInit(): void {
    this.onLoadDataCategory(this.page, this.pageSize, this.fieldToSort, this.fieldDirToSort);
  }

  /**
   * TODO: This function is working when user click select page number 5, 10, 20, ...
   */
  onPageChange(event: any) {
    let pageNumber = event;
    this.page = pageNumber;
    this.onLoadDataCategory(this.page, this.pageSize, this.fieldToSort, this.fieldDirToSort);
  }

  /**
   * TODO: This function is input page number to go to page you want select
   */
  submitGoToPage() {
    if (!this.formGoToPage.valid) {
      Swal.fire({
        title: 'Thông báo',
        text: 'Trang bạn muốn đến không tồn tại, vui lòng thử lại!',
        icon: 'warning',
        confirmButtonText: 'ok',
      }).then((result) => {
        // press any out screen
        if (result.isConfirmed) {
          this.page = 1;
          this.formGoToPage.controls['numberPage'].setValue(1);
          this.onLoadDataCategory(this.page, this.pageSize, this.fieldToSort, this.fieldDirToSort);
        }
      })
    } else {
      this.page = this.formGoToPage.controls["numberPage"].value;
      this.onLoadDataCategory(this.page, this.pageSize, this.fieldToSort, this.fieldDirToSort);
    }
  }

  @ViewChild('input') input!: ElementRef;
  ngAfterViewInit() {
    this.input.nativeElement.focus();
  }

  onKey(event: any) {
    if (event.key === 'Tab') {
      this.input.nativeElement.focus();
    }
  }

  /**
   * TODO: when press button refresh all data 
   */
  refreshAllData(): void {
    this.fieldToSort = "id";
    this.fieldDirToSort = "desc";
    this.onLoadDataCategory(this.page, this.pageSize, this.fieldToSort, this.fieldDirToSort);
    this.listCheckboxCategory = [];
  }

  /**
   * TODO: this function is load data product by category id 
   * 
   * !error: none
   * *Load data is success 
   * @param id 
   */
  onLoadDataProduct(id: any): void {
    this.apiService.selectAllByWhat('', this.W300_FIND_ALL_DATA_BY_CATEGORY_ID, id)
      .subscribe({
        next: data => {
          this.listProducts = data?.data;
        },
        error: error => {
          console.log(error);
        },
        complete: () => {
          // do something
        }
      })
  }

  /**
   * TODO: this function is load data to table 
   * 
   * !error: none
   * *Load data is success
   */
  onLoadDataCategory(offset: number, limit: number, sortField: string, sortDir: string): void {
    this.spinner.show();
    let objectNew = {
      "offset": offset - 1,
      "limit": limit,
      "sortField": sortField,
      "sortDir": sortDir
    };
    this.apiService.selectAllByWhat(JSON.stringify(objectNew), this.W100_SELECT_ALL_CATEGORY_PAGING)
      .subscribe({
        next: data => {
          console.log(data);
          this.listCatalogues = data?.data?.categories;
          this.pageLength = data?.data?.totalPage;
        },
        error: error => {
          if (error?.error?.message.includes('Page index must not be less than zero')) {
            this.spinner.hide();
            Swal.fire('Thông báo', 'Trang không được nhỏ hơn hoặc bằng 0!', 'warning');
          }
        },
        complete: () => {
          console.log(this.listCatalogues);
          setTimeout(() => {
            this.spinner.hide();
          }, 500);
          let ref = document.getElementById('keywordSearch');
          ref?.focus();
        }
      })
  }

  /**
  * TODO: this function is get all data is checked from checkbox on table
  * 
  * @param e event when click delete button
  * @param id id pass from Angular form
  */
  getCheckedOnList(e: any, id: any) {
    if (e.target.checked) {
      this.listCheckboxCategory.push(id);
    } else {
      this.listCheckboxCategory = this.listCheckboxCategory.filter(m => m != id)
    }
  }

  /**
   * TODO: this function is check validate name of category(catalogue) 
   * 
   * @param control 
   * @returns 
   */
  categoryNameValidate(control: AbstractControl): { [key: string]: boolean } | null {
    if (control.value.length > 30) {
      return { 'largeName': true };
    }
    // else if (control.value.length < 5 && control.value.length > 0) {
    //   return { 'smallName': true };
    // }
    return null;
  }


  /**
   * TODO: This function is validate all form field 
   * 
   * @param formGroup 
   */
  validateAllFormFields(formGroup: FormGroup) {
    Object.keys(formGroup.controls).forEach(field => {
      const control = formGroup.get(field);
      if (control instanceof FormControl) {
        control.markAsTouched({ onlySelf: true });
      } else if (control instanceof FormGroup) {
        this.validateAllFormFields(control);
      }
    });
  }

  /**
   * TODO: this function is used to when user input data render code (SeoURL) for insert
   */
  renderForCodeSEO() {
    this.formData.controls['code'].setValue(this.commonService.cleanAccents(this.formData.value.name));
  }

  /**
   * TODO: this function is open modal add category and focus input
   */
  openModalAddCategory() {
    let open = document.getElementById('openModalAdd');
    open?.click();
    setTimeout(() => {
      document.getElementById('nameAddCategory')?.focus()
    }, 600);
  }

  /**
   * TODO: this function is submit data & check exists before submit data to server
   * 
   * !error: none 
   * *Insert data still success
   */
  submitDataCategory(): void {
    this.apiService.selectAllByWhat(JSON.stringify(this.formData.value), this.W105_CHECK_EXISTS_CATEGORY)
      .subscribe({
        next: data => {
          if (data?.data == true && data?.status == true) {
            this.spinner.hide();
            Swal.fire('Thông báo', 'Danh mục này đã tồn tại!', 'warning');
          } else {
            this.insertDataCategoryWhenPressSubmit();
          }
        },
        error: error => {
          console.log(error.message);
        }
      })
  }

  /**
   * TODO: When check exists and continue insert
   * 
   * !error: none 
   * *Insert data still success
   */
  insertDataCategoryWhenPressSubmit(): void {
    if (!this.formData.valid) {
      this.validateAllFormFields(this.formData);
    } else {
      this.spinner.show();
      this.apiService.selectAllByWhat(JSON.stringify(this.formData.value), this.W101_INSERT_DATA_CATEGORY)
        .pipe()
        .subscribe({
          next: data => {
            if (data?.status == true) {
              this.onLoadDataCategory(this.page, this.pageSize, this.fieldToSort, this.fieldDirToSort);
            }
          },
          error: error => {
            //handle error
            console.log(error);
          },
          complete: () => {
            // reset value form
            // this.formData.controls['name'].setValue('');
            // this.formData.controls['code'].setValue('');


            // show toast check add continue
            Swal.fire({
              title: 'Thông báo',
              text: 'Bạn đã thêm danh mục thành công, bạn có muốn thêm tiếp!',
              icon: 'success',
              showCancelButton: true,
              confirmButtonText: 'Thêm tiếp!',
              cancelButtonText: 'Thoát'
            }).then((result) => {
              // press any out screen
              if (result.dismiss) {
                // reset form click cancel
                this.formData.controls['name'].setValue('');
                this.formData.controls['code'].setValue('');
                let ref = document.getElementById('cancelSubmit');
                ref?.click();
              }
              if (result.isDenied) {
                // reset form click cancel
                this.formData.controls['name'].setValue('');
                this.formData.controls['code'].setValue('');
                let ref = document.getElementById('cancelSubmit');
                ref?.click();
              }
              if (result.value) {
                this.formData.controls['name'].setValue('');
                this.formData.controls['code'].setValue('');
                // set timeout to focus input
                setTimeout(() => {
                  this.spinner.hide();
                  let ref = document.getElementById('nameAddCategory');
                  ref?.focus();
                }, 500);

              }
              let name = document.getElementById('nameAddCategory');
              name?.focus();
            })
          }
        });
    }
  }

  /**
   * TODO: this function is delete data from table with list checkbox to checked
   * 
   * !small error: none
   * *Delete data category success
   */
  deleteDataCategory(): void {
    if (this.listCheckboxCategory.length > 1) {
      Swal.fire('Cảnh báo', 'Vì là thông tin quan trọng vui lòng chọn một danh mục để thao tác và xóa!', 'warning');
    }
    if (this.listCheckboxCategory.length < 1) {
      Swal.fire('Cảnh báo', 'Vui lòng chọn những hàng mà bạn cần xóa ở cột đầu tiên trong bảng!', 'warning');
    }
    if (this.listCheckboxCategory.length == 1) {
      this.listCheckboxCategory.map(i => {
        Swal.fire({
          title: 'Cảnh báo',
          text: 'Khi xóa danh mục này, các sản phẩm quan trọng thuộc danh mục này sẽ bị xóa bạn vẫn muốn tiếp tục?',
          icon: 'error',
          showCancelButton: true,
          confirmButtonText: 'Vâng, Tôi sẽ xóa nó!',
          cancelButtonText: 'Không, giữ lại'
        }).then((result) => {
          if (result.value) {
            // console.log('day la lay danh sach cac list check box de kiem tra');
            this.getDataCategoryById(i);
          }
        })
      });
    }
  }

  /**
   * TODO: get all data category by id and check data id exists if ok then delete
   * 
   * @param id 
   */
  categoryName: string = ''; // declared data to pass data when get data from api 
  categoryId: any;
  deleteStatus: boolean = false;
  getDataCategoryById(id: any): void {
    this.apiService.selectAllByWhat('', this.W108_GET_DATA_BY_ID, id).subscribe({
      next: data => {
        // console.log('day la lay data 1 cai gi do cbi xoa', data)
        if (data?.status == true && data?.data?.deleteStatus == false) {
          this.categoryName = data?.data?.name;
          this.categoryId = data?.data?.id;
          this.deleteStatus = data?.data?.deleteStatus;
        } else {
          this.deleteStatus = data?.data?.deleteStatus;
        }
      },
      error: error => {
        // console.log(error?.error);
        if (error?.error?.message.includes('Unable to find') || error?.error?.message.includes('with id')) {
          this.itemWasDeletedByOtherUserAlert()
        } else {
          console.log(error);
        }
      },
      complete: () => {
        if (this.deleteStatus == true) {
          this.itemWasDeletedByOtherUserAlert();
        } else {
          this.onLoadDataProduct(this.categoryId);
          let ref = document.getElementById('openModalButton');
          ref?.click();
        }
      },
    })
  }

  /**
   * TODO: this function is show alert when check delete, detail or update by other people and id not exists
   */
  itemWasDeletedByOtherUserAlert() {
    Swal.fire({
      title: 'Cảnh báo',
      text: 'Danh mục này đã được xóa bởi một người khác, vui lòng nhấn bất kỳ để làm mới!',
      icon: 'warning',
      // showCancelButton: true,
      confirmButtonText: 'OK!',
      // cancelButtonText: 'Không, giữ lại'
    }).then((result) => {
      if (result.value) {
        this.onLoadDataCategory(this.page, this.pageSize, this.fieldToSort, this.fieldDirToSort);
        this.listCheckboxCategory = [];
      }
      if (result.dismiss) {
        this.onLoadDataCategory(this.page, this.pageSize, this.fieldToSort, this.fieldDirToSort);
        this.listCheckboxCategory = [];
      }
    });
  }

  /**
   * TODO: This function is delete list check box when checked
   * 
   * * checked all data to delete
   */
  deleteDataCategoryWhenCheckAll(): void {
    this.apiService.selectAllByWhat(this.listCheckboxCategory, this.W103_DELETE_DATA_CATEGORY)
      .subscribe({
        next: data => {
          if (data?.status == true) {
            this.onLoadDataCategory(this.page, this.pageSize, this.fieldToSort, this.fieldDirToSort);
          }
        },
        error: error => {
          // handle error
          console.log(error);
        },
        complete: () => {
          this.listCheckboxCategory = [];

          this.spinner.show();

          setTimeout(() => {
            this.spinner.hide();
          }, 500);

          Swal.fire(
            'Thông báo!',
            'Bạn đã xóa danh mục này thành công!',
            'success'
          )
        }
      })
  }

  /**
   * TODO: this function is delete data by id when view, and delete product then delete category
   * 
   * @param id 
   */
  continueDeleteData(id: any) {
    this.apiService.selectAllByWhat('', this.W107_DELETE_DATA_BY_ID, id)
      .subscribe({
        next: data => {
          this.onLoadDataCategory(this.page, this.pageSize, this.fieldToSort, this.fieldDirToSort);
        },
        error: error => {
          // handle error
          console.log(error.message);
        },
        complete: () => {
          this.spinner.show();

          setTimeout(() => {
            this.listCheckboxCategory = [];
            let cancelDelete = document.getElementById('cancelDelete');
            cancelDelete?.click();

            this.spinner.hide();
          }, 500);

          Swal.fire(
            'Thông báo!',
            'Bạn đã xóa danh mục này thành công!',
            'success'
          )
        }
      })
  }

  /**
   * TODO: this function is used to when user input data render code (SeoURL) for update
   */
  renderForCodeSEOUpdate() {
    this.formUpdate.controls['code'].setValue(this.commonService.cleanAccents(this.formUpdate.value.name));
  }

  /**
   * TODO: this function is pass data value for form data 
   * 
   * @param code code pass from Angular form
   */
  updateCategoryData(code: any): void {
    this.apiService.selectAllByWhat('', this.W104_DETAIL_DATA_CATEGORY, code)
      .pipe()
      .subscribe({
        next: data => {
          if (data?.status == true && data?.data?.deleteStatus == false) {
            this.formUpdate.controls['name'].setValue(data?.data?.name);
            this.formUpdate.controls['code'].setValue(data?.data?.code);
            this.formUpdate.controls['id'].setValue(data?.data?.id);
            this.deleteStatus = data?.data?.deleteStatus;
          } else {
            this.deleteStatus = data?.data?.deleteStatus;
          }
        },
        error: error => {
          // console.log(error?.error);
          if (error?.error?.message.includes('Unable to find') || error?.error?.message.includes('with id')) {
            this.itemWasDeletedByOtherUserAlert();
          } else {
            console.log(error);
          }
        },
        complete: () => {
          if (this.deleteStatus == true) {
            this.itemWasDeletedByOtherUserAlert();
          } else {
            let ref = document.getElementById('openModalUpdate');
            ref?.click();
          }
        }
      })
  }

  /**
   * TODO: this function is submit when user click button Update (chinh sua)
   * 
   * !error: none
   * *Update data is success 
   */
  saveUpdateDataCategory(): void {
    this.spinner.show();
    // check exists data category
    this.apiService.selectAllByWhat(JSON.stringify(this.formUpdate.value), this.W105_CHECK_EXISTS_CATEGORY)
      .subscribe({
        next: data => {
          if (data?.data == true && data?.status == true) {
            this.spinner.hide();
            Swal.fire('Thông báo', 'Danh mục này đã tồn tại!', 'warning');
          } else {
            // update data 
            this.apiService.selectAllByWhat(JSON.stringify(this.formUpdate.value), this.W102_UPDATE_DATA_CATEGORY)
              .pipe()
              .subscribe({
                next: data => {
                  if (data?.status == true) {
                    this.onLoadDataCategory(this.page, this.pageSize, this.fieldToSort, this.fieldDirToSort);
                  }
                },
                error: error => {
                  console.log(error);
                },
                complete: () => {
                  this.formUpdate.controls['id'].setValue('');
                  this.formUpdate.controls['name'].setValue('');
                  this.formUpdate.controls['code'].setValue('');
                  let ref = document.getElementById('cancelUpdate');
                  ref?.click();

                  setTimeout(() => {
                    this.spinner.hide();
                  }, 500);

                  Swal.fire(
                    'Thông báo!',
                    'Bạn đã chỉnh sửa danh mục này thành công!',
                    'success'
                  )
                }
              });
          }
        },
        error: error => {
          console.log(error.message);
        }
      })
  }

  /**
   * TODO: this function is show detail data from category code
   * 
   * @param code code pass from Angular form
   */
  showDetailData(code: any): void {
    this.apiService.selectAllByWhat('', this.W104_DETAIL_DATA_CATEGORY, code)
      .pipe()
      .subscribe({
        next: data => {
          if (data?.status == true && data?.data?.deleteStatus == false) {
            this.formDetail.controls['id'].setValue(data?.data?.id);
            this.formDetail.controls['name'].setValue(data?.data?.name);
            this.formDetail.controls['code'].setValue(data?.data?.code);
            this.formDetail.controls['createdDate'].setValue(moment(data?.data?.createdDate).format('DD-MM-YYYY HH:mm'));
            this.formDetail.controls['createdBy'].setValue(data?.data?.createdBy);
            if (data?.data?.modifiedDate == data?.data?.createdDate) {
              this.formDetail.controls['modifiedDate'].setValue('Chưa được cập nhật');
              this.formDetail.controls['modifiedBy'].setValue('Chưa được cập nhật');
            } else {
              this.formDetail.controls['modifiedDate'].setValue(moment(data?.data?.modifiedDate).format('DD-MM-YYYY HH:mm'));
              this.formDetail.controls['modifiedBy'].setValue(data?.data?.modifiedBy);
            }
            this.deleteStatus = data?.data?.deleteStatus;
          } else {
            this.deleteStatus = data?.data?.deleteStatus;
          }
        },
        error: error => {
          // console.log(error?.error);
          if (error?.error?.message.includes('Unable to find') || error?.error?.message.includes('with id')) {
            this.itemWasDeletedByOtherUserAlert();
          } else {
            console.log(error);
          }
        },
        complete: () => {
          // if delete status == true is item was deleted
          if (this.deleteStatus == true) {
            this.itemWasDeletedByOtherUserAlert();
          } else {
            // else then load data product by category id
            this.onLoadDataProduct(this.formDetail.value.id);
            // openModalDetail: open modal get detail data
            let ref = document.getElementById('openModalDetail');
            ref?.click();
          }
        }
      })
  }

  /**
   * TODO: this function is sort category name based on alphabet 
   */
  sortAlphabetCategoryName() {
    this.spinner.show();
    this.fieldToSort = "code";
    this.fieldDirToSort = "asc";
    this.onLoadDataCategory(this.page, this.pageSize, this.fieldToSort, this.fieldDirToSort)
    setTimeout(() => {
      this.spinner.hide();
    }, 500);
    let focusSearch = document.getElementById('keywordSearch');
    focusSearch?.focus();
  }

  /**
   * TODO: sort default option data
   */
  sortDefaultOption() {
    this.fieldToSort = "id";
    this.fieldDirToSort = "desc";
    this.onLoadDataCategory(this.page, this.pageSize, this.fieldToSort, this.fieldDirToSort);
    this.listCheckboxCategory = [];
  }

  /**
   * TODO: sort data created date option 
   */
  sortCreatedDateOption() {
    this.spinner.show();
    this.onLoadDataCategory(this.page, this.pageSize, "created_date", "asc")
    setTimeout(() => {
      this.spinner.hide();
    }, 500);
    let focusSearch = document.getElementById('keywordSearch');
    focusSearch?.focus();
  }

  /**
   * TODO: sort updated date option
   */
  sortUpdatedDateOption() {
    this.spinner.show();
    this.onLoadDataCategory(this.page, this.pageSize, "modified_date", "asc")
    setTimeout(() => {
      this.spinner.hide();
    }, 500);
    let focusSearch = document.getElementById('keywordSearch');
    focusSearch?.focus();
  }

  /**
   * TODO: sort data alphabet category name reverse A-Z
   */
  sortAlphabetCategoryNameReverse() {
    this.spinner.show();
    this.fieldToSort = "code";
    this.fieldDirToSort = "desc";
    this.onLoadDataCategory(this.page, this.pageSize, this.fieldToSort, this.fieldDirToSort)
    setTimeout(() => {
      this.spinner.hide();
    }, 500);
    let focusSearch = document.getElementById('keywordSearch');
    focusSearch?.focus();
  }

  /**
   * TODO: this function is select option to show list data 5 record, 10 record,...
   * 
   */
  onOptionsSelected(event: any) {
    const value = event.target.value;
    this.pageSize = value;
    this.onLoadDataCategory(this.page, this.pageSize, this.fieldToSort, this.fieldDirToSort);
  }

  /**
   * TODO: search data category (catalogue)
   */
  keywordSearch: any;
  myControl = new FormControl();
  startSearch: boolean = false;
  public keyUp = new Subject<KeyboardEvent>();
  private subscription!: Subscription;
  searchDataCatalogue() {
    this.myControl.valueChanges.pipe(
      debounceTime(500),
      // switchMap(changedValue => this.productService.search_Products(changedValue)),
    ).pipe(distinctUntilChanged()).subscribe(search => {
      if (search == '') {
        console.log('refresh lại');
      } else {
        console.log(search);
      }
    });
  }

  /**
   * TODO: when close modal detail then confirm delete data
   */
  closeModalDetail() {
    let ref = document.getElementById('cancelDetail');
    let refDel = document.getElementById('cancelDelete');
    let focusSearch = document.getElementById('keywordSearch');
    ref?.focus();
    focusSearch?.click();
    refDel?.click();
  }

  /**
   * TODO: delete category data by id when press delete button icon in table
   * 
   * @param id 
   */
  deleteCategoryDataById(id: any) {
    Swal.fire({
      title: 'Cảnh báo',
      text: 'Khi xóa danh mục này, các sản phẩm quan trọng thuộc danh mục này sẽ bị xóa bạn vẫn muốn tiếp tục?',
      icon: 'error',
      showCancelButton: true,
      confirmButtonText: 'Vâng, Tôi sẽ xóa nó!',
      cancelButtonText: 'Không, giữ lại'
    }).then((result) => {
      if (result.value) {
        this.getDataCategoryById(id);
      }
    })
  }

  /**
   * TODO: sort alphabet in column 
   */
  isReverse: boolean = false;
  sortAlphabetColumn() {
    this.isReverse = !this.isReverse;
    if (this.isReverse) {
      this.fieldToSort = "code";
      this.fieldDirToSort = "desc";
      this.onLoadDataCategory(this.page, this.pageSize, this.fieldToSort, this.fieldDirToSort);
    } else {
      this.fieldToSort = "code";
      this.fieldDirToSort = "asc";
      this.onLoadDataCategory(this.page, this.pageSize, this.fieldToSort, this.fieldDirToSort)
    }
  }

  /**
   * TODO: sort date column
   */
  sortDateColumn() {
    this.isReverse = !this.isReverse;
    if (this.isReverse) {
      this.fieldToSort = "created_date";
      this.fieldDirToSort = "desc";
      this.onLoadDataCategory(this.page, this.pageSize, this.fieldToSort, this.fieldDirToSort);
    } else {
      this.fieldToSort = "created_Date";
      this.fieldDirToSort = "asc";
      this.onLoadDataCategory(this.page, this.pageSize, this.fieldToSort, this.fieldDirToSort);
    }
  }

  /**
   * TODO: comfirm when close modal then user input but close modal
   */
  confirmBeforeCloseAddCatalogue() {
    if (this.formData.controls['name'].value == '') {
      this.formData.controls['name'].setValue('');
      this.formData.controls['code'].setValue('');
      let ref = document.getElementById('cancelSubmit');
      ref?.click();
      let search = document.getElementById('keywordSearch');
      search?.focus();
    } else {
      Swal.fire({
        title: 'Cảnh báo',
        text: 'Bạn đã chưa lưu những gì đã nhập bạn vẫn muốn tiếp tục thoát?',
        icon: 'warning',
        showCancelButton: true,
        confirmButtonText: 'Thoát!',
        cancelButtonText: 'Không, giữ lại'
      }).then((result) => {
        if (result.value) {
          this.formData.controls['name'].setValue('');
          this.formData.controls['code'].setValue('');
          // this.formData.reset();
          let ref = document.getElementById('cancelSubmit');
          ref?.click();
          let search = document.getElementById('keywordSearch');
          search?.focus();
        }
      })
    }
  }
}
